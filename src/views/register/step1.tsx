import React, { useCallback } from "react";
import { Form, Input, Select, Avatar, message as Message } from "antd";
import { ModelInterface, useBookingData, useUserData } from "../../models/data";
import { NextButton } from "../../components/common/PageHeaderWrapper";
import { ViewMetaDate } from "../../lib/types/View";
import { SendCodeResponse, useSendCode } from "../../lib/request/api";

export const metaData: ViewMetaDate = {
  header: {
    title: "Sign in",
    subTitle: "with your phone #",
  },
};

const flag =
  "data:image/png;base64,";

const { Option } = Select;

const selectBefore = (
  <Select
    style={{ backgroundColor: "#cecece" }}
    defaultValue="+62"
    className="select-before"
  >
    <Option value="+62">
      <Avatar
        className={"icon"}
        src={flag}
        shape="square"
        style={{ marginRight: 5 }}
      />
      +62
    </Option>
  </Select>
);

export default function Step1(props: any) {
  console.log("step 1 props...................", props);
  const bookingData: ModelInterface = useBookingData();
  const user: ModelInterface = useUserData();
  const { step1 } = bookingData.value;
  const sendCode = useSendCode();
  const onFinish = useCallback(async (args) => {
    console.log("step 1 args", args);
    const response = (await sendCode(args)) as SendCodeResponse;
    const { message, ...rest } = response;
    Message.success("Code send to your phone number.");
    bookingData.value.step1 = rest;
    bookingData.update();
    user.value.username = "rest";
    user.update();
    props.history.push("step2");
  }, []);

  return (
    <>
      <Form
        name="basic"
        labelCol={{ span: 8 }}
        wrapperCol={{ span: 16 }}
        initialValues={step1}
        onFinish={onFinish}
        onFinishFailed={() => {}}
      >
        <Form.Item
          label="Phone"
          name="phone"
          rules={[{ required: true, message: "Please input phone" }]}
        >
          <Input addonBefore={selectBefore} />
        </Form.Item>
        <NextButton label={"Send Code"} />
      </Form>
    </>
  );
}
